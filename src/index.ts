#!/usr/bin/env node
/**
 * Ramp MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their Ramp OAuth credentials as Bearer token
 * in the format: Bearer <RAMP_CLIENT_ID>:<RAMP_CLIENT_SECRET>
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Ramp corporate cards, bills, transactions, reimbursements, and spend management through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Ramp OAuth credentials as the Bearer token in format CLIENT_ID:CLIENT_SECRET. No credentials are stored on this server.',
      header: 'Authorization: Bearer <RAMP_CLIENT_ID>:<RAMP_CLIENT_SECRET>',
      howToGetKey:
        'Create an OAuth application at https://app.ramp.com/developer-settings to get your Client ID and Client Secret.',
    },
    configTemplate: {
      mcpServers: {
        ramp: {
          ...
          headers: {
            Authorization: 'Bearer <RAMP_CLIENT_ID>:<RAMP_CLIENT_SECRET>',
          },
        },
      },
    },
...
<head>
  ...
  <title>Ramp MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Ramp MCP Server</h1>
    <p class="subtitle">Access Ramp corporate cards, bills, transactions, reimbursements, and spend management through MCP tools.</p>

    <div class="card">
      <h2>How to get your Ramp API credentials</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.ramp.com" target="_blank" rel="noopener">Ramp</a>.</li>
        <li>Navigate to <strong>Developer Settings</strong>.</li>
        <li>Create a new <strong>OAuth Application</strong>.</li>
  ...
  <script>
    ...
    function buildConfig(id, secret) {
      const creds = (id || 'CLIENT_ID') + ':' + (secret || 'CLIENT_SECRET');
...
function extractBearerToken(req: express.Request): { clientId: string; clientSecret: string } | null {
  ...
  const clientId = token.substring(0, colonIndex);
  const clientSecret = token.substring(colonIndex + 1);
...
function createMCPServer(client: RampClient): Server {
  const server = new Server(
    { name: 'ramp-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  if (!creds) {
    res.status(401).json({
      error: 'Missing or invalid Authorization header. Use: Bearer <RAMP_CLIENT_ID>:<RAMP_CLIENT_SECRET>',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new RampClient(creds.clientId, creds.clientSecret);
...
<head>
  ...
  <title>Authorize \u2014 Ramp MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Ramp MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Ramp MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Ramp MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides RAMP_CLIENT_ID:RAMP_CLIENT_SECRET)`);
});

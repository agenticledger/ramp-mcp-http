#!/usr/bin/env node
/**
 * Ramp MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Ramp secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Ramp
 ...
 *      `Authorization: Bearer <ramp-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Ramp key, construct the client.
...
const SLUG = 'ramp';
const NAME = 'Ramp MCP Server';
const VERSION = '2.0.0';

/** Construct a RampClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): RampClient {
  const o = JSON.parse(credential); return new RampClient(o.clientId, o.clientSecret);
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'Ramp Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Ramp API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Ramp key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <ramp-api-key>',
    ],
  });
});
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Ramp isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Ramp is not connected for this caller yet. Connect it once via the link below (paste your Ramp API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Ramp is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-ramp-api-key>' },
      });
      return;
    }
